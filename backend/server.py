...
app = Flask(__name__)
bcrypt = Bcrypt(app)
CORS(app, resources={r"/*": {"origins": "*"}})# Make sure your password special characters are URL-encoded (e.g., '@' becomes '%40')
...
def setup_chatbot():
    ...
    api_key = os.getenv('GROQ_CHATBOT_API_KEY') # Fixed: removed repeated characters
    if not api_key:
...
def patient_signup():
    ...
    # Validate email domain (must be Gmail)
    gmail_pattern = r'^[a-zA-Z0-9._%+-]+@gmail\.com$'
    if not re.match(gmail_pattern, email):
        return jsonify({
            "message": "Only @gmail.com email addresses are allowed. Please use a valid Gmail account."
        }), 400
    ...
    if len(email_parts) != 2 or not email_parts[0] or email_parts[1] != 'gmail.com':
        return jsonify({
            "message": "Invalid email format. Please use a valid @gmail.com address."
        }), 400
    ...
    try:
        ...
        session_token = bcrypt.generate_password_hash(
            f"{email}{datetime.now(timezone.utc)}"
        ...
        return jsonify({"message": "An error occurred while creating your account. Please try again."}), 500
...
def patient_login():
    ...
    try:
        ...
        if patient and bcrypt.check_password_hash(patient.password_hash, password):
            session_token = bcrypt.generate_password_hash(f"{email}{datetime.now(timezone.utc)}").decode('utf-8').replace('/', '').replace('.', '')
            session = UserSession(patient_id=patient.id, session_token=session_token,expires_at=datetime.now(timezone.utc) + timedelta(days=7))
            db.session.add(session)
            db.session.commit()
...
def predict_diabetes_subtype():
    try:
        required_models = [
            'diabetes_subtype_classifier',
            'diabetes_risk_model',
            'diabetes_preprocessor',
            'diabetes_cluster_map'
        ]
        for m in required_models:
            if m not in models or models[m] is None:
                return jsonify({"error": "Diabetes subtype models are not loaded properly."}), 500
        ...
        # --- Cluster Stats (from cluster centers) ---
        cluster_center = models['diabetes_risk_model'].cluster_centers_[risk_cluster]
        cluster_stats = {
            "mean_HbA1c": float(cluster_center[0]),
            "mean_BMI": float(cluster_center[1]),
            "mean_Age": float(cluster_center[2])
        }

        # --- Explanation ---
        explanation = (
            f"The model predicts {prediction} with {max(prediction_proba)*100:.1f}% confidence. "
            f"Risk assessment: {risk_level} risk level. "
        ...
        response = {
            ...
            'cluster_stats': cluster_stats   # ✅ Included in response
        }

        # --- Save to DB (report_json will now include cluster_stats) ---
        if user_id:
...
@app.route('/check_diabetes_models', methods=['GET'])
def check_diabetes_models():
    status = {}
    required_models = [
        'diabetes_subtype_classifier',
        'diabetes_risk_model',
    ...
    for m in required_models:
        status[m] = "Loaded ✅" if m in models and models[m] is not None else "Not Loaded ❌"
